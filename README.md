...
Installing `vtk` from the `freecad/label/devs` channel can be achieved by adding `freecad/label/devs` to your channels with:

```
conda config --add channels freecad/label/devs
conda config --set channel_priority strict
```

Once the `freecad/label/devs` channel has been enabled, `vtk, vtk-base, vtk-io-ffmpeg` can be installed with `conda`:
...
```
conda search vtk --channel freecad/label/devs
```

or with `mamba`:

```
mamba search vtk --channel freecad/label/devs
```
...
mamba repoquery search vtk --channel freecad/label/devs

# List packages depending on `vtk`:
mamba repoquery whoneeds vtk --channel freecad/label/devs

# List dependencies of `vtk`:
mamba repoquery depends vtk --channel freecad/label/devs
```
